 ...
 * glass pane effect is only for the container and not the entire frame. By
 * default the glass pane is invisible. When the DisablePanel is disabled then
 * the glass pane is made visible. In addition:
 *
 * a) All MouseEvents will now be intercepted by the glass pane. b) The panel is
 * removed from the normal focus traveral policy to prevent any component on the
 * panel from receiving focus. c) Key Bindings for any component on the panel
 * will be intercepted.
 *
 ...
 *
 * The background color of the glass pane should use a color with an alpha value
 * to create the disabled look.
 *
 * The other approach to disabling components on a Container is to recurse
 * through all the components and set each individual component disabled. This
 * class supports two static methods to support this functionality:
 *
 * a) disable( Container ) - to disable all Components on the Container b)
 * enable ( Container ) - to enable all Components disabled by the disable()
 * method. That is any component that was disabled prior to using the disable()
 * method method will remain disabled.
 */
public class DisabledPanel extends JPanel {
    ...
    public static void disable(Container container) {
        List<JComponent> components = SwingUtils.getDescendantsOfType(
                JComponent.class, container, true);
    ...
    static class GlassPane extends JComponent {
        ...
        public GlassPane() {
            ...
            Color background = new Color(base.getRed(), base.getGreen(),
                    base.getBlue(), 128);
        ...
        /*
         * The component is transparent but we want to paint the background to
         * give it the disabled look.
         */
        @Override
     ...
     * processing.
     */
    static class DisabledEventQueue extends EventQueue implements
            WindowListener {
        ...
        public void addPanel(DisabledPanel panel) {
            ...
            EventQueue current = Toolkit.getDefaultToolkit()
                    .getSystemEventQueue();
        ...
        protected void dispatchEvent(AWTEvent event) {
            ...
            if (event.getID() == KeyEvent.KEY_TYPED
                ...
                for (DisabledPanel panel : panels.keySet()) {
                    Window panelWindow = SwingUtilities
                            .windowForComponent(panel);

                    // A binding was found so just return without dispatching
                    // it.
         ...
         */
        private boolean searchForKeyBinding(DisabledPanel panel,
                KeyStroke keyStroke) {
